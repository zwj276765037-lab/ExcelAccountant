...
8. 在方案左侧勾选需要的方案，确认输出目录后点击“输出勾选方案”。输出近似方案前必须再次确认风险。

搜索完成时不会自动生成文件。精确和近似方案都需要用户手动勾选后才能输出。近似方案的实际合计可能不等于目标金额，程序会强制弹出风险确认，并在文件名和审计表中显著标记“近似”及差额。

## 输出文件

- 每套完整方案是一个独立文件：`原文件名_方案001.xlsx`、`原文件名_方案002.xlsx` 等。
- 近似方案使用 `原文件名_近似方案001.xlsx` 命名，审计表记录实际合计、有符号差额和绝对差额，必须人工复核。
